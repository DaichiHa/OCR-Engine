...
# 【超・抽出特化型プロンプト (ver.GPT-4o)】
ULTRA_EXTRACTION_PROMPT = """
あなたは歴史的統計資料のデジタル化専門プログラムです。
画像内の情報を、以下の『絶対規則』に従って100%正確に書き起こしてください。

[絶対規則]
1. 出力はMarkdown形式の書き起こしのみ。挨拶・解説は厳禁。
2. 統計表は行列を1マスもズラさずMarkdownテーブルで完全に再現せよ。
3. 漢字はすべて原典のまま（旧字体・異体字をそのまま維持。例：港灣、價額、數量、噸、圓）。
4. 数値、コンマ（,）、単位（石、斤、噸、圓等）を1文字のミスもなく記載せよ。
5. 縦書きの文章は日本語の正しい順序で、助詞（ニ、ヲ、ハ）も含め正確に再現せよ。
            ...
            messages=[
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": ULTRA_EXTRACTION_PROMPT},
                        {
                            "type": "image_url",
                            "image_url": {
