...
for p in sorted(ops.iterdir()):
    if p.is_file():
        ...
        if p.name in ['page_010_inventory.csv','page_010_cleanup_report.txt','page_010_archive_summary.json','page_010_archive.zip']:
            continue
...
with open(summary, 'w', encoding='utf-8') as f:
    json.dump({'moved': moved, 'groups': [d.name for d in outdir.iterdir() if d.is_dir()]}, f, indent=2, ensure_ascii=False)
