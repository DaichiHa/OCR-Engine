...
for p in files:
    ...
    rows.append({'filename': p.name, 'path': str(p), 'size_bytes': size, 'is_image': is_image, 'width': width, 'height': height, 'text_preview': preview})
    if size == 0:
...
with open(csv_path, 'w', newline='', encoding='utf-8') as csvf:
    writer = csv.DictWriter(csvf, fieldnames=['filename','path','size_bytes','is_image','width','height','text_preview'])
    writer.writeheader()
    for r in rows:
        writer.writerow(r)
...
with open(report_path, 'w', encoding='utf-8') as rf:
    ...
    keep = [r['filename'] for r in rows if ('normalized' in r['filename'] or 'ensemble' in r['filename'] or 'clean.txt' in r['filename'] or 'ollama.suggested' in r['filename'] or 'sample' in r['filename'])]
    for k in keep:
        rf.write('- ' + k + '\n')
