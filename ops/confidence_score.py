...
def readability_score(text: str) -> float:
    ...
    if not lines:
        return 0.0
    total_words = sum(len(re.findall(r"\w+", l)) for l in lines)
...
def score_file(p: Path, group_counts: dict) -> dict:
    ...
    prefix = re.sub(r'\.(clean|normalized|ppocr|tess|txt|ollama|suggested).*', '', name, flags=re.I)
...
def main():
    ...
    for p in cand:
        name = p.name
        prefix = re.sub(r'\.(clean|normalized|ppocr|tess|txt|ollama|suggested).*', '', name, flags=re.I).lower()
