...
ocr = ONNXPaddleOcr(use_angle_cls=True, use_gpu=False, use_dml=False, use_openvino=False)

def process(img_path: Path):
    img = cv2.imdecode(np.fromfile(str(img_path), dtype=np.uint8), cv2.IMREAD_COLOR) if False else cv2.imread(str(img_path))
    if img is None:
