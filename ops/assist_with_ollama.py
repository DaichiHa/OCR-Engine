...
from ollama_helper import load_config, generate
...
def main():
    ...
    candidates = [ln for ln in txt if noisy_score(ln) >= args.threshold and len(ln.strip())>6]
    ...
    print(json.dumps({'status':'ok', 'out': str(out_path), 'candidates': len(candidates)}))
