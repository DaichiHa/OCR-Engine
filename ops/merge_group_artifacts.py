...
TEXT_EXTS = ['.txt', '.tess.txt', '.ppocr.txt', '.rapid.txt', '.clean.txt', '.normalized.txt', '.ollama.suggested.txt', '.json']
...
for group in sorted([d for d in la.iterdir() if d.is_dir()]):
    ...
    priority = ['.clean.txt', '.normalized.txt', '.tess.tiled.psm11.txt', '.tess.txt', '.rapid.txt', '.ppocr.txt', '.ollama.suggested.txt', '.txt', '.json']
    def score(p):
        name = p.name.lower()
    ...
    merged_summary.append({'group': group.name, 'canonical': canonical.name, 'merged': str(merged_path), 'count_files': len(files)})
