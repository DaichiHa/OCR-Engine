...
for psm in PSMS:
    for oem in OEMS:
        ...
        cmd = [
            sys.executable,
            os.path.join(ROOT, "ops", "mini_runner.py"),
            "--page",
            IMG,
            "--out",
            outdir,
        ...
        start = time.time()
        with open(os.path.join(outdir, "out.txt"), "wb") as fout, open(
            os.path.join(outdir, "err.txt"), "wb"
        ) as ferr:
            try:
                proc = subprocess.run(
                    cmd, env=env, stdout=fout, stderr=ferr, timeout=120
                )
        ...
        rows.append(
            {
                ...
                "oem": oem,
                "jp_rate": jp or "0",
                "noise_rate": noise or "999",
                "elapsed_s": str(elapsed),
                "md": md_files[0] if md_files else "",
            }
        )
...
with open(summary_csv, "w", newline="", encoding="utf-8") as csvf:
    writer = csv.DictWriter(
        csvf, fieldnames=["psm", "oem", "jp_rate", "noise_rate", "elapsed_s", "md"]
    )
    writer.writeheader()
    for r in rows:
        writer.writerow(r)
...
rows_sorted = sorted(
    rows,
    key=lambda r: (-float(r["jp_rate"]), float(r["noise_rate"]), float(r["elapsed_s"])),
)
print("\n--- Top 5 PSM/OEM ---")
