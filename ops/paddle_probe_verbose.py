...
for img in imgs:
    ...
    if FORCE_TESSERACT_ONLY:
        ...
        if tesseract:
            ...
            try:
                ...
                subprocess.run([tesseract, str(img), out_base], check=True, timeout=TIMEOUT_SECONDS)
    ...
    if not worker.exists():
        ...
        break
    try:
        subprocess.run([sys.executable, str(worker), str(img)], check=True, timeout=timeout)
    except subprocess.TimeoutExpired:
