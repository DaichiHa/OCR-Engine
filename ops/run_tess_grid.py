...
def main(smoke=False, assist=False):
    ...
    for idx, combo in enumerate(combos, start=1):
        ...
        cmd = [
            ...
            "--in",
            "ops/page_010_sample.png",
            "--out",
            str(out_img),
            "--scale",
            str(params["scale"]),
            "--clahe-clip",
            str(params["clahe_clip"]),
            "--clahe-tile",
            str(params["clahe_tile"]),
            "--denoise-h",
            str(params["denoise_h"]),
        ]
        if params["deskew"]:
            cmd.append("--deskew")
        if params["binarize"]:
