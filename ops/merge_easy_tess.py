...
def easy_items(easy_json_path):
    ...
    for it in data:
        ...
        items.append({'box': box, 'text': str(it.get('text','')), 'conf': float(it.get('prob',0.0)), 'source': 'easy'})
    return items
    ...
    for e in easy:
        ...
        if best:
            ...
            if e['conf'] >= max(0.0, t.get('conf',0.0)):
                merged.append({'box': e['box'], 'text': e['text'], 'conf': e['conf'], 'source':'easy'})
            else:
                merged.append({'box': t['box'], 'text': t['text'], 'conf': t['conf'], 'source':'tess'})
        else:
            merged.append(e)
...
def main():
    ...
    if easy_path is None:
        ...
        easy_path = os.path.join(os.path.dirname(outdir), 'easyocr_outputs_page_010_full', 'full_easyocr.json')
