...
def process_file(p: Path):
    ...
    text4 = re.sub(
        r"re\.findall\([^\)]*,\s*l\)\s*for\s+line\s+in",
        lambda m: m.group(0).replace(", l)", ", line)"),
        text2,
    )
    if text4 != text2:
        text2 = text4
    ...
    text5 = re.sub(
        r"for\s+line\s+in[\s\S]*?lines\.append\(line\)",
        lambda m: m.group(0).replace("append(line)", "append(line)"),
        text2,
    )
    if text5 != text2:
        text2 = text5
